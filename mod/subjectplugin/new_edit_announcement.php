<?php 
	...
	if (has_capability('mod/subjectplugin/view.php:read', $context)) 
	{
		require_login($course, true, $cm);
	}
	add_to_log($course->id, 'subjectplugin', 'new announcement', "new_edit_announcement.php?id={$cm->id}", $subjectplugin->name, $cm->id);

	/// Print the page header
	
	$PAGE->set_url('/mod/subjectplugin/new_edit_announcement.php', array('id' => $cm->id));
	...
	if($operation=="n")
	{
	  $insertSQL = sprintf("INSERT INTO mdl_announcement (id, name, annoucement_dt,archived, archived_dt, url)
		...
		if(!mysql_query($insertSQL, $annoncement))
			...
			//redirect("$CFG->wwwroot/mod/subjectplugin/view.php?id=$id");		
	}else if($operation=="e")
	{					   		
